...
d3.csv('../data/census_states_pct.csv', d3.autoType)
    .then(data => {
        ...
        svg.selectAll("rect")
            ...
            .on('mouseout', function () {
                d3.select(this)
                    ...
                    .delay(100)
                    .attr("fill", "slategray") //bars will turn color when mouse leaves
            })
        ...
        const svgFem = d3.select("#second-container")
            .append("svg")
            .attr("width", width)
            .attr("height", height)


        const xScaleFem = d3.scaleLinear()
            .domain([0, d3.max(data, d => d.Fem_ConstructionExtractionandMaintenance)]) //d3 max = function expecting an array - can pass in an accessor function
            .range([margin, width - margin])


        // yScale
        const yScaleFem = d3.scaleBand()
            .domain(data.map(d => d.Statistics))
            .range([height - margin, margin])
            .paddingInner(.5)

        // colorScale
        const colorScaleFem = d3.scaleOrdinal()
            .domain(xScaleFem.domain()) //same as putting the [0,d3.max(data, d => d.TotalPopBySex)]
        ...
        /* AXES */
        // xAxis
        const xAxisFem = d3.axisBottom(xScale)
        svg.append("g")
            .attr("class", "axis")
            .attr("transform", `translate(0,${height - margin})`) //works - xAxis shows at bottom
            .call(xAxisFem)
        ...
        // yAxis
        const yAxisFem = d3.axisLeft(yScaleFem)
        svgFem.append("g")
            .attr("class", "axis")
            .attr("transform", `translate(${margin},0)`) //moves the yAxis over 60px, 0 
            .call(yAxisFem)
        ...
        svgFem
            ...
            .attr("y", height / 16) //higher the denominator, higher the text moves up pg
            .attr("text-anchor", "middle")
            .text("Females in construction extraction and maintenance")
            .style("font-size", "24px")
            .style("text-decoration", "underline")
            .attr("fill", "darkcyan")
        ...
        // AXIS LABELS
        svgFem.selectAll("labels")
            .data(data)
            .enter()
            .append("text")
            .attr("class", "labels")
            .text(d => d.Fem_ConstructionExtractionandMaintenance)//returns formatted string - if # then adds commas
            .attr("x", d => xScaleFem(d.Fem_ConstructionExtractionandMaintenance))
            .attr("y", d => yScaleFem(d.Statistics) + yScaleFem.bandwidth() / 1) //dividing by 2 puts the count the middle of the bar
            .style("font-size", "10px")

        // BARS
        svgFem.selectAll("rect")
            .data(data)
            .enter()
            .append("rect")
            //.join("rect")
            .attr("class", "bar")
            .attr("height", yScaleFem.bandwidth()) //girth of bars 
            .attr("width", d => xScaleFem(d.Fem_ConstructionExtractionandMaintenance) - margin) //=> shorthand for function - must return a value
            .attr("x", d => margin) //bars will start at the margin
            .attr("y", d => yScaleFem(d.Statistics))
            .on('mouseover', function () {
                d3.select(this)
                    .attr("fill", d => colorScaleFem(d.Fem_ConstructionExtractionandMaintenance))
            })
            .on('mouseout', function () {
                d3.select(this)
                    .transition("colorfade")
                    .delay(100)
                    .attr("fill", "darkviolet") //bars will turn color when mouse leaves
            })
            .append("title") //adds tooltip (text) too all "rect" elements on mouseover
            .text(d => (d.Statistics + ": " + d.Fem_ConstructionExtractionandMaintenance))
        //d => (d.Statistics + " population is " + d.Fem_ConstructionExtractionandMaintenance.toLocaleString())  replaces (function(d) { return (d.Statistics + " population is " + d.TotalPopulationBySex.toLocaleString()) })

        // d3.select(".value-sort").on("click", function () {
        //     data.sort(function (a, b) {
        //         return d3.descending(b.Fem_ConstructionExtractionandMaintenance, a.Fem_ConstructionExtractionandMaintenance);
        //     })
        //     yScaleFem.domain(data.map(d => d.Statistics)) //(d => d.Statistics) replaces (function(d) {return d.Statistics;});

        //     svgFem.selectAll(".bar")
        //         .transition()
        //         .duration(700) //changes how fast the bars shift
        //         .attr("y", function (d, i) {
        //             return yScaleFem(d.Statistics);
        //         }) //will move the bars


        //     svgFem.selectAll(".labels") //select class labels to move
        //         .transition()
        //         .duration(500)
        //         .attr("y", function (d, i) {
        //             return yScale(d.Statistics) + yScaleFem.bandwidth() / 1;
        //         }) //will move the labels
        // })
        ...
        //     yScaleFem.domain(data.map(d => d.Statistics))

        //     svgFem.selectAll(".bar")
        //         .transition()
        //         .duration(700)
        //         .attr("transform", function (d, i) {
        //             return "translate (" + (yScaleFem(d.Statistics) + yScaleFem.bandwidth() / 2 - 8) + "," + (height + 20) + ")"
        //         })
        ...
        //         .duration(700)
        //         .attr("transform", function (d, i) {
        //             return "translate (" + (yScaleFem(d.Statistics));
        //         })
        // })
